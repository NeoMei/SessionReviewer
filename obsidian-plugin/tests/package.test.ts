...
describe("Obsidian plugin package", () => {
  it.skipIf(process.platform === "win32")("packages only installable assets with matching versions reproducibly", async () => {
    ...
    const build = (dist: string): void => {
      execFileSync("bash", ["scripts/build-obsidian-plugin.sh", "0.2.0", dist], {
        cwd: repository,
    ...
    build(first);
    build(second);
    const archiveName = "session-reviewer-obsidian-0.2.0.zip";
    ...
    expect(manifest).toMatchObject({ id: "session-reviewer", version: "0.2.0" });
