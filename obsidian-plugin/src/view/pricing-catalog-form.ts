...
export function pricingCatalogForm(price:PricingSnapshotV1,actions:PricingActions):HTMLElement {
 const root=element("div");const open=button("查询 ModelPriceWatch 价格",{"data-action":"query-catalog"});const feedback=element("p",{attrs:{role:"status"}});root.append(open,feedback);
 open.addEventListener("click",()=>{if(open.disabled||!actions.catalog||!actions.acceptCatalog)return;open.disabled=true;feedback.textContent="正在读取公开价格目录…";
  void actions.catalog().then(catalog=>{
   ...
   form.addEventListener("submit",event=>{event.preventDefault();if(submit.disabled||!form.reportValidity()||!catalog.listings.some(r=>r.listing_id===select.value))return;
    ...
    submit.disabled=true;cancel.disabled=true;feedback.textContent="正在核对历史价格并保存…";void actions.acceptCatalog!(input).then(()=>{feedback.textContent="已保存定价结果，正在重新读取；不适用的价格仍显示待定。";}).catch(()=>{feedback.textContent="目录价格未保存；请刷新账本后重试，输入已保留。";submit.disabled=false;cancel.disabled=false;});
   });root.append(form);
