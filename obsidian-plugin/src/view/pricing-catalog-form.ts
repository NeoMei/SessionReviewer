...
export function pricingCatalogForm(price:PricingSnapshotV1,actions:PricingActions):HTMLElement {
 ...
 const root=element("div",{className:"sr-v4-pricing-control"});const open=button(replacing?"查询并替换当前价格":"查询 ModelPriceWatch 价格",{"data-action":"query-catalog"});const feedback=element("p",{attrs:{role:"status"}});root.append(open,feedback);
 open.addEventListener("click",()=>{if(open.disabled||!actions.catalog||!actions.acceptCatalog)return;open.disabled=true;feedback.textContent="正在读取公开价格目录…";
  void actions.catalog().then(catalog=>{
   ...
   const form=element("form",{className:"sr-v4-pricing-form",attrs:{"aria-label":"确认目录价格"}});form.append(element("p",{text:`为 ${price.provider} / ${price.session_id} 的 ${price.billed_model_id} 选择价格。请按实际服务商和调用模式核对，名称相似不会自动匹配。`}));
   ...
   const submit=button("确认路由并保存目录价格",{type:"submit"});const cancel=button("取消",{});cancel.addEventListener("click",()=>{form.remove();open.disabled=false;});form.append(element("div",{className:"sr-form-actions"},[submit,cancel]));
