...
export function renderDecisionCandidates(candidates:AgentAnnotationEntryV1[],decisions:DecisionV4[],transition?:DecisionTransition, sources?: {projectId:string;generationId:string;evidence:DecisionCandidateEvidence[];loadConversation?:ConversationLoader}):HTMLElement & {dispose:()=>void} {
 ...
 const root=element("section",{className:"sr-decision-candidates",attrs:{"aria-label":"决策候选"}},[element("header",{className:"sr-decision-section-heading"},[element("h3",{text:"待确认建议"}),element("span",{className:"sr-decision-count",text:String(candidates.filter(c=>c.status==="pending").length)})]),element("p",{className:"sr-decision-description",text:"建议不会自动成为项目决策；确认前可修改内容。"})]) as HTMLElement & {dispose:()=>void};
 root.dispose=()=>{for(const viewer of viewers)viewer.dispose();root.replaceChildren();};
 const pending=candidates.filter(c=>c.status==="pending");const history=element("details",{className:"sr-decision-history"},[element("summary",{text:`历史候选 · ${candidates.length-pending.length}`})]);
 ...
 for(const candidate of candidates){
  ...
  const card=element("article",{className:"sr-card sr-decision-candidate"},[element("header",{className:"sr-decision-candidate-heading"},[element("h4",{text:input?.title??"候选内容无法读取"}),element("span",{className:"sr-decision-badge",text:labels[candidate.status],attrs:{"data-status":candidate.status}})]),element("p",{className:"sr-decision-rationale",text:input?.rationale??"请重新提取建议。"})]);
  const footer=element("div",{className:"sr-decision-candidate-footer"});const actions=element("div",{className:"sr-decision-actions"});
  const evidence=sources?.evidence.find(entry=>entry.candidate_id===candidate.id);
  if(sources){const viewer=renderDecisionEvidence(sources.projectId,sources.generationId,evidence,sources.loadConversation);viewers.push(viewer);footer.append(viewer);}
  const evidenceAvailable=!sources||!!evidence&&!evidence.error_code&&evidence.evidence_refs.length>0;
  const feedback=element("p",{className:"sr-decision-feedback",attrs:{role:"status"}});const editor=element("div",{className:"sr-decision-editor"});
  const action=(key:DecisionCandidateAction,label:string)=>{const control=button(label,{"data-action":`${key}-decision-candidate`});control.className=key==="confirm"?"sr-decision-primary":key==="not_decision"?"sr-decision-quiet":"sr-decision-secondary";control.disabled=!transition||(key==="confirm"&&(!input||!evidenceAvailable));control.addEventListener("click",()=>{
  ...
  footer.append(actions);card.append(footer,editor,feedback);(candidate.status==="pending"?root:history).append(card);
