...
export function loadMarkdownSnapshot(input: { review: string; history: string; ledger: string; index: string }): MarkdownSnapshot {
  ...
  try {
    ...
    if (value.changedDocuments.length > 0 || value.changedFields.length > 0) return { kind: "pending_edit", value };
    if (ledger.review_sha256 !== sha256Text(input.review) || ledger.history_sha256 !== sha256Text(input.history)) return { kind: "pending_edit", value };
    return { kind: "public_valid", value, index };
  } catch (error) {
