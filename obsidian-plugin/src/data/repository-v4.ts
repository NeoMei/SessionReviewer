...
export type MarkdownSnapshot =
  | { kind: "public_valid"; value: MarkdownRead; index: SessionIndexV1; ledger: MachineLedgerV4 }
  | { kind: "pending_edit"; value: MarkdownRead; ledger: MachineLedgerV4 }
...
export function loadMarkdownSnapshot(input: { review: string; history: string; ledger: string; index: string }): MarkdownSnapshot {
  ...
  try {
    ...
    if (value.changedDocuments.length > 0 || value.changedFields.length > 0) return { kind: "pending_edit", value, ledger };
    if (ledger.review_sha256 !== sha256Text(input.review) || ledger.history_sha256 !== sha256Text(input.history)) return { kind: "pending_edit", value, ledger };
    return { kind: "public_valid", value, index, ledger };
  } catch (error) {
