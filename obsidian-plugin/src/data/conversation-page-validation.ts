...
export function validateConversationPage(value: unknown): ConversationPageV1 {
...
function text(value: unknown, path: string, max: number): string {
  if (typeof value !== "string" || utf8Bytes(value) > max) throw new Error(`${path} must be bounded text`);
  return value;
}
...
function cursor(value: unknown, path: string): void { if (value !== null && (typeof value !== "string" || value.length === 0 || utf8Bytes(value) > 4096 || value.includes("\0"))) throw new Error(`${path} must be a bounded cursor`); }
