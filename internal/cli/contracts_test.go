...
func TestParseDecisionContractAcceptsExactAllowlist(t *testing.T) {
	...
	tests := [][]string{
		...
		{"extract", "status", "--project-id", "project-p", "--data-dir", dataDir, "--json"},
...
func TestProblemEntryAndPlacementContracts(t *testing.T) {
	dataDir := filepath.Join(t.TempDir(), "problems")
	root := []string{"candidate", "transition", "--project-id", "p", "--candidate-id", "c", "--expected-candidate-revision", "1", "--expected-problem-map-revision", "0", "--expected-review-sha256", contractTestSHA, "--action", "apply_root", "--json"}
	request, err := ParseProblemContract(root)
	if err != nil || request.Action != "apply_root" || request.TargetProblemID != "" {
		t.Fatalf("explicit root request=%+v err=%v", request, err)
	}
	withTarget := append(append([]string(nil), root[:len(root)-1]...), "--target-problem-id", "target", "--json")
	if _, err := ParseProblemContract(withTarget); err == nil {
		t.Fatal("apply_root accepted a target")
	}

	request, err = ParseProblemContract([]string{"placement", "request", "--project-id", "p", "--candidate-id", "c", "--expected-candidate-revision", "2", "--expected-problem-map-revision", "3", "--expected-generation-id", "generation-1", "--data-dir", dataDir, "--json"})
	if err != nil || request.Command != "placement" || request.Subcommand != "request" || request.ExpectedGenerationID != "generation-1" || request.ExpectedCandidateRevision != 2 || request.ExpectedProblemMapRevision != 3 || request.DataDir != dataDir {
		t.Fatalf("placement request=%+v err=%v", request, err)
	}
	request, err = ParseProblemContract([]string{"placement", "status", "--project-id", "p", "--job-id", "job-1", "--data-dir", dataDir, "--json"})
	if err != nil || request.JobID != "job-1" {
		t.Fatalf("placement status=%+v err=%v", request, err)
	}
	request, err = ParseProblemContract([]string{"placement", "status", "--project-id", "p", "--candidate-id", "c", "--data-dir", dataDir, "--json"})
	if err != nil || request.CandidateID != "c" || request.JobID != "" {
		t.Fatalf("placement active status=%+v err=%v", request, err)
	}
	request, err = ParseProblemContract([]string{"placement", "cancel", "--project-id", "p", "--job-id", "job-1", "--expected-revision", "4", "--data-dir", dataDir, "--json"})
	...
	for _, args := range [][]string{
		...
		if _, err := ParseProblemContract(args); err == nil {
			t.Fatalf("accepted invalid placement args: %v", args)
		}
	}
}
