...
type DecisionOperationInput struct {
...
// RenderDecisionOperation creates the exact three-file publication plan for a
// trusted formal-decision operation. The authenticated Session index guards
// the generation and remains byte-for-byte outside the write set.
func RenderDecisionOperation(in DecisionOperationInput) (RenderPlan, error) {
	...
	for _, path := range paths {
		if _, ok := in.ExpectedFiles[path]; !ok {
			return RenderPlan{}, errors.New("decision operation requires every public preimage")
		}
	}
	...
	if index.Digest != in.Index.Digest || index.ProjectID != in.Presentation.ProjectID || index.GenerationID != in.Presentation.GenerationID || index.ProjectViewDigest != in.Presentation.ProjectViewDigest {
		return RenderPlan{}, errors.New("decision operation index guard mismatch")
	}
	pair, err := reviewv4.RenderMarkdownDecisionOperation(in.Presentation, oldLedger, in.Pending)
	if err != nil {
		return RenderPlan{}, err
	}
	ledger := in.Ledger
	...
	ledger.ReviewSHA256, ledger.HistorySHA256 = decisionSHA(pair.Review), decisionSHA(pair.History)
	...
	return RenderPlan{ProjectID: in.Presentation.ProjectID, GenerationID: in.Presentation.GenerationID, ProjectViewDigest: in.Presentation.ProjectViewDigest, Files: []FilePlan{
		{Relative: reviewv2.ReviewRelativePath, Expected: cloneDecisionBytes(in.ExpectedFiles[reviewv2.ReviewRelativePath]), ExpectedExists: true, Desired: pair.Review, Mode: 0o644},
		{Relative: reviewv2.HistoryRelativePath, Expected: cloneDecisionBytes(in.ExpectedFiles[reviewv2.HistoryRelativePath]), ExpectedExists: true, Desired: pair.History, Mode: 0o644},
		{Relative: filepath.ToSlash(reviewv2.MachineLedgerRelativePath), Expected: cloneDecisionBytes(in.ExpectedFiles[reviewv2.MachineLedgerRelativePath]), ExpectedExists: true, Desired: ledgerBody, Mode: 0o600},
	}}, nil
}

func decisionSHA(body []byte) string { sum := sha256.Sum256(body); return hex.EncodeToString(sum[:]) }
func cloneDecisionBytes(body []byte) []byte {
