...
- [ ] RED successful-page binding regressions from the independent audit: a same-identity first response beginning at25 rather than0,100items for limit25, a total different from the selected index's indexed_event_count, and a nonterminal25/50page with null next_cursor must all be rejected. Validate nonempty cursor topology, nonempty cursor strings, page item count against the request limit, first-request origin and inclusion of the requested anchor. The renderer must compare each response total with the authenticated selected index entry before cache or display, including initial and cached pages. Do not decode opaque cursors in the plugin. Preserve existing Go half-open ranges and legitimate final short pages.
...
- [ ] Also exercise directly supplied loaders with `cliUnavailable=true` at the rendered host boundary: summary and conversation must remain uncalled even if callbacks exist. Controller external `no-cli-qa.mjs` at87cd026 reports summary1 while the host declares CLI unavailable (retained-event fix separately covers events). Use one explicit capability gate in the host/options path and preserve legitimate source-unavailable retained-summary access when CLI is available; do not use raw-source availability as a global gate.
...
- [ ] Controller functional follow-up: when CLI or event loader is unavailable, disable the event ordinal input and submit button instead of offering a no-op action. Apply the same existing no-readable-event condition (zero count or absent authenticated view digest), keep eligible retained events enabled when CLI is available, and retain the runtime no-query guard. Do not remove the local cancellation callback, which performs no private read. Add focused availability-state tests; keyboard no-CLI probe must see disabled controls before testing the separate recovery link.
