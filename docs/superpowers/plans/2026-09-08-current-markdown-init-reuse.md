...
2026-09-09 integration ruling: real scan accepts private-memory safe IDs outside the legacy generated project-16hex shape. For current-format reuse only, authenticate the four-file ID and require the existing private-memory safe token (`^[a-z0-9][a-z0-9._-]{0,127}$`); legacy/fresh identity generation and config-fragment validation remain unchanged. A complete exact existing mapping plus safe existing private project root takes a read-only reuse path, never `ensureProjectSyncState` or fragment publication. Missing/incomplete mapping or missing/unsafe private root requires recovery; do not repair it as a side effect of reuse. This avoids widening the legacy writer or creating private proof from public documents. Cost: incomplete mappings need separate explicit recovery, not silent completion.

Frozen Gate A inventory ruling: focused gate reproduced exactly two new production import edges across its four existing targets: `internal/project -> internal/reviewv4` for the pure shared draft parser and `internal/project -> regexp` for current safe-ID validation. Add only these two sorted records to `testdata/zero-token/gate-a-production-import-edges.txt` after confirming the source diff. Do not modify dependency-path, dangerous-capability, process-site or fallback inventories. Rerun Gate A; fail and investigate any additional drift. Cost is maintaining two explicit deterministic dependency records, not permission for new Agent/network/process behavior.
...
- [ ] Preserve the current four-file preimages and pinned physical Project/Vault identities from preflight through locked validation and final reuse check. Follow the current-only no-write ruling above: current reuse never invokes scaffold/config mutation callbacks or publishes a changed mapping. Detect concurrent creation, deletion, goal edit, index replacement and root redirection via the existing afterLock boundary; return `ErrInitializationStateChanged` and preserve the changed bytes. Do not accept a newly changed draft merely because both old/new text are structurally valid. Keep mutation-callback safeguards and regressions for unchanged legacy/fresh paths; do not add fake mutation callbacks to a read-only reuse path.
