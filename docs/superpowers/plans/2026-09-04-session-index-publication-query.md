...
**Status:** Task17 flow repairs are independently reviewed and frozen5d6c398 passed all seven local commands (2026-09-06). Final broad review still found two Important defects: block-frontmatter comment loss and unreadable-candidate discovery failure; both have controller reproduction evidence, including the real isolated Obsidian discovery stall. A single combined final correction is underway; corrected source needs scoped review, fresh complete verification and native acceptance. Prior candidate receipts are historical, not proof of the next source. See `docs/verification/2026-09-05-v4-human-markdown.md`. No next feature/integration step is promoted from this state. Native cross-platform CI and real legacy migration are excluded by the user's scope decision, not passed tests. No merge/push/release or automatic real rescan. The checklist below retains original requirements; later checkpoints govern amended delivery scope.
