...
**Status:** amended implementation and local acceptance complete at tested source `cf8a39a` (2026-09-06). Task17 flow repairs and final block-comment/discovery corrections independently reviewed; final scoped review has 0 Critical / 0 Important / 0 new Minor. All seven ordered frozen checks passed, including ordinary large-Session, prescribed race, exact zero-token Gate A and plugin256tests. New matching plugin bundle passed isolated native unreadable-candidate and all-CLI-absent startup/refresh/history acceptance; same-candidate CLI154/154no-op verified all8hashes/mtimes unchanged. See `docs/verification/2026-09-05-v4-human-markdown.md`. Known in-scope debt is closed; original Tasks4–7 may resume as separate feature work and are not already implemented. Cross-platform CI and real legacy migration are user-excluded, not passed. No merge/push/release, production installation or automatic real rescan. Historical checklist below is superseded where the amended scope and final checkpoint explicitly say so.
