...
**Task completion checkpoint (2026-09-08):** Implementation `eae9683` plus controller-found Provider-clear correction `6010623` completed the bounded checklist below. Independent task review and fix re-review approved; controller full check381 tests/lint/build and rebuilt real-renderer browser assertions passed. Native current-build acceptance and broader S07 private-search/provider workflows remain open. See `docs/verification/2026-09-07-spec-restoration.md` for RED/GREEN and acceptance limits.
...
- Modify `obsidian-plugin/src/view/project-view.ts` and `presentation.ts` only for bounded state-patch forwarding and merge against the latest per-project state; `main.ts` remains unchanged.
...
**Cross-leaf persistence clarification:** Add optional `saveStatePatch?: (patch: Partial<Omit<V4ViewState, 'projectId'>>) => void | Promise<void>` at shell/wrapper boundary, retaining `saveState` fallback for standalone callers. Session changes send only `{ sessionBrowser }` without redraw. Other actions send their explicit changed fields, including any initialized selected identity. ProjectEvolutionView merges patches into `currentV4State(projectId)` immediately before normalization and persistence. Test both directions: Session edits preserve another leaf's newer non-Session fields, and tab/problem actions preserve another leaf's newer Session filters. No live cross-leaf UI synchronization is required.
