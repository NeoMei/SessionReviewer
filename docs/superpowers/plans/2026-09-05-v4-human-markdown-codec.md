...
Execution:
- [x] Reproduce both defects with actual failing committed regression tests before source edits, recording exact commands/output and root causes separately.
- [x] Implement minimal boundary and deletion fixes, self-review and run focused regressions during iteration. Before source commit run full affected syncdoc/syncproject/publication tests once; record covering tests, RED/GREEN, files, commits and concerns in the task report.
- [x] Independent task review (spec + quality), with scoped fixes/re-review if needed. No task completion before validated findings are closed.
- [x] Freeze candidate source; final whole-branch review plus the exact ordered seven-command M9 local sequence (ordinary full Go including large Session; vet; tidy -diff; exact Gate A; prescribed race exclusion; diff --check; full plugin check). Do not assemble green status from earlier candidates.
- [x] Rebuild the fixture-only CLI and repeat approved synthetic status -> scan -> sync -> status, checking complete artifact hashes/mtime and zero-token output. Existing native no-CLI evidence can be retained only with unchanged plugin source/dependency/bundle proof; this backend repair requires authenticated test coverage on its own candidate.
- [x] Update tracked verification and both plan checkpoints with exact results and remaining gates. Retain local evidence; no integration, publishing or unrelated feature work without separate direction.

### Final checkpoint (2026-09-06)

Task17 and this known-debt batch complete at tested source `cf8a39a`. Four task fix rounds closed flow compositions; final broad review's block-comment/discovery findings were repaired in one combined wave and independently closed with no new finding. Required scalar controls exposed terminal-LF drift; controller-approved v4-private encoding correction preserves typed values without altering legacy encoder/dependencies/schema. Final covering and frozen seven-command receipts passed, plugin256tests/19files. The new matching plugin bundle was actually rerun in isolated Obsidian with an unreadable candidate and all CLI runtimes unavailable; valid project, readonly refresh and native history worked, public hashes unchanged. Same-candidate synthetic CLI154/154scan/sync no-op preserved all8hashes/mtimes with0tokens.

Known in-scope debt is closed, not a guarantee of undiscovered-defect absence. Original Session Index Task3 amended local gate may hand back to separate Tasks4–7; those features are not completed here. Cross-platform CI and real legacy migration remain excluded by user decision. No merge/push/release, production plugin/CLI/Vault/config change or automatic real rescan. Preserve branch/worktree and local evidence. Final source identity, commands, actual times/cache disclosure, failure history and native/CLI hashes are recorded in `docs/verification/2026-09-05-v4-human-markdown.md`; subsequent docs-only checkpoint commits are not represented as new source-test runs.
