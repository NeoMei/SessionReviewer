...
Capability clarification from Task2 review: both explicitly selected page modes, including an explicit current-view selector, require `minimum_reader_version=0.4.3`. Omitted requests retain0.4.0. Standalone conversation-page Go/TS/schema consumers support both valid capabilities; request-aware runner/renderer enforce the expected conditional floor and reject downgrades or unexpected selected-page contracts. Shared summary/event capability acceptance remains unchanged. Add direct explicit-current/full-body and cursor-separation coverage alongside historical selection.
