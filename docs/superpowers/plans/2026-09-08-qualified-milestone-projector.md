...
Consumer clarification2026-09-09: each emitted dependency carries the full ordered turn-ID set of its exact selected chain; filtering concerns which snapshots are emitted, not changing a snapshot's turn inventory. Supported machine-kind vocabulary is `machine_verification|machine_commit|machine_release|machine_deployment|machine_version`; strongest category changes the displayed kind but never the fixed `qualified-turn-v1` ID family. Typed rollback remains a separately required source/retained qualifier task because no supported producer exists yet; do not infer it from prose.
