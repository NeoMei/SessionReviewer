...
- [ ] **Implement independent summary component.** Maintain request epoch plus complete binding key; ignore outdated results, preserve retry only for current binding, and invalidate on dispose. Keep the initial summary compact: five native, initially closed block disclosures with their titles and total/shown/omitted counters always visible; expanding one reveals its bounded items and does not expand the other blocks. Empty blocks visibly say no captured matching facts. Label server `shown` as returned summary entries, not as currently expanded DOM entries. Entries show text, timestamps and source IDs in native disclosures. Test keyboard expansion and independent block state. Use a polite loading/error status. No model summaries or invented labels indicating resolved/passed without explicit facts.
