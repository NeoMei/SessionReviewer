...
历史页能力声明补充：显式快照查询（包括显式选择当前快照）的索引页和消息页声明 `minimum_reader_version=0.4.3`；省略选择器的默认页仍为 `0.4.0`。页解析器支持这两种能力，携带请求上下文的调用方还须核对对应能力，不得接受显式查询返回的旧能力页，或把限定查询页当成默认查询页。summary/event 等其他只读合同不随之升级。
