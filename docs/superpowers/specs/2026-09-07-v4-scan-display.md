...
`session-reviewer inspect session-events --project-id ID --provider codex --session-id ID --expected-generation-id ID --limit 25 --json [--cursor TOKEN | --anchor ORDINAL]`

Anchor is the one-based ordinal in canonical display order, not a raw source sequence number.
