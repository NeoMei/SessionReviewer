...
The test creates all Project, Vault, data, manual-merge, and backup roots under Go test temporary directories. Its only durable output is the content-free result `8/8 steps passed; 3/3 isolated conflict actions passed`; it emits no generated absolute path or fixture content.
