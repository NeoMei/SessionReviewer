...
The replay initializes a temporary real Git repository, authenticates its root and common directory, and uses the production ProjectProbe to read branch, HEAD, dirty count, remote identity, version-file hash, and required-file hash. Cancellation and complete Project/Vault tree snapshots prove the probe and scan do not change paths, bytes, modes, sizes, or file/directory modification times. Captured project commands are decoded but never executed. Reviewed fixtures freeze both the complete transitive production import-path union and the complete direct-import edge-set digest for Darwin amd64/arm64, Linux amd64, and Windows amd64, including standard-library and third-party packages; the check runs with network access disabled and a test-temporary Go cache, and any path or import-edge delta fails the gate until reviewed. A local call-site audit excludes Agent/model packages and network-client imports and permits only `net.ParseIP` in the ProjectProbe remote parser; the transitive standard-library closure therefore still honestly contains `net` internals. The production ProjectProbe receives its existing `RunGit` seam bound to one identity-checked real Git binary. The seam rejects non-Git executables and non-approved argv before execution, fixes the authenticated project cwd and reduced Git environment, records every call, and proves all 35 observed calls are the five production-approved read-only Git operations repeated across seven probes. Complete user text/tool output canaries do not enter the private store, and the only safe verbose test log is:
