...
The replay initializes a temporary real Git repository, authenticates its root and common directory, and uses the production ProjectProbe to read branch, HEAD, dirty count, remote identity, version-file hash, and required-file hash. Cancellation and complete Project/Vault tree snapshots prove the probe and scan do not change paths, bytes, modes, sizes, or file/directory modification times. Captured project commands are decoded but never executed. Human-readable fixtures freeze canonical target masks separately for every transitive import path and direct import edge on Darwin amd64/arm64, Linux amd64, and Windows amd64, including standard-library and third-party packages; removing a path or edge from one target fails even when another target retains it. The check runs with module-network lookup disabled and test-temporary Go caches, never rewrites fixtures, and requires review for every record delta.

For every selected local production source file, a second readable manifest freezes the target mask, import alias, file, enclosing function, symbol or suspicious method receiver, and occurrence for `os/exec`, Process symbols, `syscall`, `x/sys`, `net` namespaces, plugin loading, `unsafe`, and reflection surfaces. The audit rejects dot imports and any local generated, assembly, cgo, `#cgo`, or `go:linkname` surface in the complete local package closure. This is a source-level import/selector and high-risk-method audit; it does not claim that standard-library packages are free of internal networking or assembly. The only approved local process implementation records are the `os/exec` import plus `exec.CommandContext` and `command.Run` inside ProjectProbe's `defaultGitRunner`; no package-wide process exception exists.

Gate A gives ProjectProbe an intentionally unusable fallback Git path and injects the recording `RunGit` seam, so every successful probe command must select that seam. The recorder itself executes one separately identity-checked real Git binary, rejects non-Git executables and non-approved argv before execution, fixes the authenticated project cwd and reduced Git environment, and accounts every attempt. All 35 observed production-probe calls are the five approved read-only Git operations repeated across seven probes, with zero rejected production calls. Complete user text/tool output canaries do not enter the private store, and the only safe verbose test log is:
