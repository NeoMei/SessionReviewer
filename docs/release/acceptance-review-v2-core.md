...
ProjectView reduction uses a deterministic chronological k-way merge and explicit bounds for witnessed state, pending and completed recovery links, phase boundaries, event references, Session-derived records, and module candidates. Module selection uses Space-Saving admission and exposes `estimated_activity`, `error_upper_bound`, and `counts_complete`; evicted/truncated counts are never presented as exact. Tests cover more than 65,536 summaries, interleaved chronology, simultaneous pending/completed recovery residency, and a late heavy hitter.
