...
无需 Go 工具链时，从 [最新 GitHub Release](https://github.com/NeoMei/SessionReviewer/releases/latest) 下载与平台对应的归档、Obsidian 插件和 `SHA256SUMS`。CLI 归档解压后包含 CLI、README、许可证以及完整的 `skill/session-reviewer` 包。当前源码候选版本为 `0.4.3`：

- Apple Silicon Mac：`session-reviewer_0.4.3_darwin_arm64.tar.gz`
- Intel Mac：`session-reviewer_0.4.3_darwin_amd64.tar.gz`
- Windows x64：`session-reviewer_0.4.3_windows_amd64.zip`
- Obsidian：`session-reviewer-obsidian-0.4.3.zip`
...
发布包中的 `session-reviewer-obsidian-0.4.3.zip` 只包含三个可安装文件。解压后，将整个 `session-reviewer` 目录放到：
...
Codex 附图消息保留有界文字，覆盖不完整仍明确提示；没有退出码的历史输出不会被当作验证成功。未知费用不会冒充零。当前源码是本地候选，不代表已经正式发布；最新公开下载与源码版本独立。最终视觉确认、准确 Codex 会话恢复、远端 CI、发布与安装仍分别验收。

升级前备份回顾/历史和本机接受状态，显式重扫以启用新的问答分段规则；不删除人工内容绕过初始化或迁移。详见 [0.4.3 候选说明](docs/release/0.4.3.md)。
...
```bash
./scripts/build-release.sh 0.4.3 dist
```

Windows PowerShell 使用：

```powershell
.\scripts\build-release.ps1 -Version 0.4.3 -Dist dist
```

Obsidian 插件包可独立构建：

```bash
./scripts/build-obsidian-plugin.sh 0.4.3 dist
```

```powershell
.\scripts\build-obsidian-plugin.ps1 -Version 0.4.3 -Dist dist
```
