...
- Windows x64：`session-reviewer_0.2.2_windows_amd64.zip`
- Obsidian：`session-reviewer-obsidian-0.2.2.zip`
...
```bash
./scripts/build-release.sh 0.2.2 dist
```

Windows PowerShell 使用：

```powershell
.\scripts\build-release.ps1 -Version 0.2.2 -Dist dist
```

Obsidian 插件包可独立构建：

```bash
./scripts/build-obsidian-plugin.sh 0.2.2 dist
```

```powershell
.\scripts\build-obsidian-plugin.ps1 -Version 0.2.2 -Dist dist
```
