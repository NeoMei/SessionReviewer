...
## Review fix round 1/5 (FIX_BASE `e410cd2`)

### Findings addressed

1. Newly encoded custom frontmatter units previously retained yaml.v3's LF endings even when the accepted document's frontmatter used CRLF. `rewriteV4Frontmatter` now derives the physical newline convention from the existing frontmatter opener and converts only newly serialized unit bytes. Existing frontmatter spans and mixed body bytes remain untouched.
2. Shell placeholders previously used actual-document ordinals, so two independently parsed physical block orderings could produce indistinguishable shell units and silently reconstruct the wrong presentation. Placeholders now bind to the stable tuple `field/generated + full entity + full field name`, with entity and field encoded as lowercase hexadecimal. This preserves allowed physical presentation movement without changing ledger structure or globally rejecting block order. Known duplicates, unknown identities, and injected internal tokens fail closed; generated bytes stay attached to their authenticated identity.

No reviewv4 order rejection was added. The controller-selected stable-identity approach intentionally leaves physical block movement valid while making that movement visible to semantic shell merge.

### Behavioral RED

Before either production fix, the two required regressions failed on the reviewed behavior:

```text
$ go test ./internal/syncdoc -run 'TestV4(ShellCustomFrontmatterUsesCRLF|StablePlaceholders)' -count=1
--- FAIL: TestV4ShellCustomFrontmatterUsesCRLFForChangedAndAddedUnits (0.01s)
    v4_units_test.go:148: CRLF frontmatter edit changed physical bytes
        ...
        want: "---\r\nid: review-project-p\r\nentity_type: project-review\r\nproject_id: project-p\r\nschema_version: 4\r\ndocument_format: review-markdown-v1\r\nrevision: 1\r\ngeneration_id: generation-1\r\nminimum_reader_version: 0.4.1\r\nminimum_writer_version: 0.4.1\r\ncustom_owner: 修改\r\ncustom_team: codec\r\n---\r\n"
--- FAIL: TestV4StablePlaceholdersPreserveFormalIdentityAcrossPhysicalReordering (0.00s)
    --- FAIL: TestV4StablePlaceholdersPreserveFormalIdentityAcrossPhysicalReordering/two_human_fields (0.00s)
        v4_units_test.go:188: independent physical reordering was hidden by ordinal placeholders
    --- FAIL: TestV4StablePlaceholdersPreserveFormalIdentityAcrossPhysicalReordering/human_and_generated (0.00s)
        v4_units_test.go:188: independent physical reordering was hidden by ordinal placeholders
FAIL
FAIL github.com/neomei/SessionReviewer/internal/syncdoc 0.532s
```

These are behavioral failures: both documents parsed, but the rendered physical bytes violated the newline contract and ordinal tokens erased a permitted physical change.

### Focused GREEN

After the fixes, the original regressions plus caller-level merge and injection coverage passed:

```text
$ go test ./internal/syncdoc ./internal/sync -run 'TestV4(ShellCustomFrontmatterUsesCRLF|StablePlaceholders|MergeKeepsReorderedShell)' -count=1
ok  github.com/neomei/SessionReviewer/internal/syncdoc  0.530s
ok  github.com/neomei/SessionReviewer/internal/sync     0.751s
```

The tests prove CRLF change/add behavior with an unrelated mixed-LF body byte preserved; human/human and human/generated block movement; Project field plus Vault shell/field three-way merge; exact identity-preserving reconstruction; and duplicate/unknown placeholder rejection.

### Final affected-package verification

Per the fix brief, the affected packages were run once and no full repository suite was run:

```text
$ go test ./internal/reviewv4 ./internal/syncdoc ./internal/sync -count=1
ok  github.com/neomei/SessionReviewer/internal/reviewv4  0.704s
ok  github.com/neomei/SessionReviewer/internal/syncdoc   0.760s
ok  github.com/neomei/SessionReviewer/internal/sync      29.348s
```

### Fix files and self-review

- `internal/syncdoc/v4_shell.go`
- `internal/syncdoc/v4_units.go`
- `internal/syncdoc/v4_units_test.go`
- `internal/sync/v4_merge_test.go`
- `.superpowers/sdd/2026-09-05-v4-human-markdown-codec/task-4-report.md`

Self-review confirmed that the identity encoding is unambiguous and derived only from already validated bounded fields; the generated flag participates in identity; stable placeholders do not encode business graph or revision state; reconstruction still verifies every known token exactly once; and newline conversion applies only to fresh yaml.v3 output, never to retained physical spans. No external write, dependency, release, push, merge, plugin, Vault, plan, or controller-ledger change was made.
