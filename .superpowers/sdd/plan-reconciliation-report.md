...
| Index privacy test rejected its own `ContentHash` contract | Field/column checks explicitly allow `ContentHash`/`content_hash`, `SourceHash`/`source_hash`, and named SHA-256 identities while rejecting generic content/narrative/title/body/evidence fields and scanning SQLite/WAL/SHM/quarantine bytes for forbidden narrative canaries. |
...
| Native receipts and candidate-affecting commits formed a cycle | Native automation is committed without collecting final receipts; all remaining code/tests/workflows/README/packaged inputs are committed through Task 9; a post-task gate builds the candidate at that exact HEAD, dispatches the committed native workflow, and forbids any later commit without discarding/restarting evidence. |
| Final archive rehearsal omitted the Skill archive | Every archive install in the final gate passes both `--archive` and mandatory `--skill-archive` with the same checksum manifest. |
| Tag push had no native run-ID input channel | Publication is a separate `workflow_dispatch` bound to exact tag `v0.1.0`, exact commit, and required `native_run_id`; tag push alone never publishes or bypasses receipt validation. |
